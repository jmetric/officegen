...
module.exports = {
  ...
  _getCell: function (val, options, idx, row_idx) {
    if ((typeof val === 'object')) {
      //Cell-specific formatting passed in
      options = (val.hasOwnProperty('options')) ? val.options : options;
      val = (val.hasOwnProperty('val')) ? val.val : val;
    }
    ...
    var cellObject = {
      "a:tc": {
        "a:txBody": {
          ...
          "a:p": {
            "a:pPr": {"@algn": options.align ? (options.align[idx] ? options.align[idx] : 'ctr') : 'ctr'},
            "a:r": {
              "a:rPr": {
                "@lang": "en-US",
                "@sz": "" + font_size * 100,
    ...
    if(options.hasOwnProperty('fillColor')){
      /*
      cellObject["a:tc"]["a:tcPr"] = {
        "a:shd": {
          "@a:val": "clear",
          "@a:color": "auto",
          "@a:fill" :  options.fillColor
        }
      };
      */
      ...
      cellObject["a:tc"]["a:tcPr"]["a:solidFill"] = {
        "a:srgbClr": {
          "@val": options.fillColor
        }
      };

      //Apply to paragraph range
      /*
